...
def prepare_dataset(filename):
    ...
    x_cv, x_test, y_cv, y_test = train_test_split(x_,
                                                  y_,
                                                  test_size=0.50,
                                                  random_state=80)
...
def train_plot_poly(model,
    ...
    plt.plot(degrees, cv_mses, marker='o', c='b', label='CV MSEs')
    plt.plot(degrees,
             np.repeat(baseline, len(degrees)),
             linestyle='--',
             label='baseline')
...
def train_plot_diff_datasets(model, files, max_degree=10, baseline=None):
    ...
    for file in files:
        ...
        x_train, y_train, x_cv, y_cv, x_test, y_test = prepare_dataset(
            file['filename'])
...
def train_plot_learning_curve(model,
                              x_train,
                              y_train,
                              x_cv,
                              y_cv,
    ...
    # Plot the results
    plt.plot(num_samples_train_and_cv,
             train_mses,
             marker='o',
             c='r',
             label='training MSEs')
